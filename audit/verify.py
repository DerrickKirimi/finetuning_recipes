...
def main():
    ...
    for task, name in [("sft", "paperbd/paper_instructions_300K-v1"),
        ...
        commands.append([sys.executable, "audit/data_boundaries.py", "--train", folder + "/train.jsonl",
                         "--test", folder + "/test.jsonl", "--output", str(out / task)])
    commands += [
        [sys.executable, "audit/map_citations.py", "--reference-map", str(Path(args.reference_map).resolve()),
         "--revision", args.revision, "--output", str(out)],
        [sys.executable, "audit/batches.py", "--sample", str(out / "sft/sample.jsonl"),
        ...
        [sys.executable, "audit/dpo_tokens.py", "--sample", str(out / "dpo/sample.jsonl"),
        ...
        [sys.executable, "-m", "pytest", "-q", "-o", f"cache_dir={out / 'pytest-cache'}",
         "--basetemp", str(out / "pytest-tmp"), "audit/test_audit.py", "tests/reasoning", "reasoning/tests",
    ...
    files = [*Path(__file__).parent.glob("*.py"), repo / "pyproject.toml", repo / "uv.lock",
             repo / "check_env.sh", repo / "audit/pyproject.toml", repo / "audit/uv.lock",
